...
class DatabaseSessionManager:
    ...
    async def create_session(self) -> AsyncGenerator[AsyncSession, None]:
        async with self.session_maker() as session:
            try:
                ...
                logger.error(f"Ошибка при создании сессии базы данных: {error}")
                raise
            finally:
                await session.close()
    ...
    async def transaction(self, session: AsyncSession) -> AsyncGenerator[None, None]:
        try:
            ...
            logger.exception(f"Ошибка транзакции: {error}")
            raise
            ...
            async def wrapper(*args, **kwargs):
                async with self.session_maker() as session:
                    try:
                        ...
                        await session.rollback()
                        logger.error(f"Ошибка при выполнении транзакции: {error}")
                        raise
                    finally:
                        await session.close()
